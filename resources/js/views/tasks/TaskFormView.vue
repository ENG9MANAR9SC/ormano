<template>
    <v-container fluid class="pa-6">
        ...
        <div class="text-h5 font-weight-bold text-secondary mb-6">{{ isEdit ? 'Edit task' : 'New task' }}</div>
        ...
        <v-form v-else @submit.prevent="submit">
            <v-card rounded="xl" elevation="0" class="border pa-4 pa-md-6">
                <v-row>
                    ...
                    <v-col cols="12" md="6">
                        <v-select v-model="form.status" :items="enums.task_statuses" item-title="label" item-value="value" label="Status *" variant="outlined" rounded="lg" />
                    </v-col>
                    <v-col cols="12" md="6">
                        <v-select v-model="form.priority" :items="enums.task_priorities" item-title="label" item-value="value" label="Priority *" variant="outlined" rounded="lg" />
                    </v-col>
                    <v-col cols="12" md="6">
                        <v-text-field v-model="form.due_date" type="date" label="Due date" variant="outlined" rounded="lg" />
                    </v-col>
                    <v-col cols="12" md="6">
                    ...
                    <v-col cols="12" md="6">
                        <v-select v-model="form.assigned_to" :items="options.users" item-title="name" item-value="id" label="Assigned to" variant="outlined" rounded="lg" clearable />
                    </v-col>
                    <v-col cols="12" md="6">
                ...
                <div class="d-flex flex-wrap ga-3 mt-4">
                    ...
                    <v-btn variant="outlined" rounded="xl" :to="{ name: 'tasks.index' }">Cancel</v-btn>
                </div>
            </v-card>
        </v-form>
    </v-container>
...
const enums = reactive({ task_statuses: [], task_priorities: [] });
const options = reactive({ cases: [], users: [] });
const form = reactive({
    title: '', description: '', status: 'open', priority: 'medium', due_date: '', case_id: null, assigned_to: null, created_by: null,
});
...
function applyPayload(payload) {
    ...
    form.priority = payload.priority ?? 'medium';
    form.due_date = payload.due_date ?? '';
...
async function boot() {
    ...
    error.value = '';
    try {
        const [eRes, fRes] = await Promise.all([http.get('/api/v1/meta/enums'), http.get('/api/v1/meta/task-form')]);
        enums.task_statuses = eRes.data.task_statuses || [];
        enums.task_priorities = eRes.data.task_priorities || [];
        options.cases = fRes.data.cases || [];
        options.users = fRes.data.users || [];
        if (isEdit.value && taskId.value) {
            const { data } = await http.get(`/api/v1/tasks/${taskId.value}`);
            applyPayload(data.data);
        }
    } catch (e) {
        error.value = e.response?.data?.message || 'Failed to load task form.';
    } finally {
...
async function submit() {
    ...
    try {
        if (isEdit.value) {
            await http.put(`/api/v1/tasks/${taskId.value}`, payload());
            router.push({ name: 'tasks.show', params: { id: taskId.value } });
        } else {
            const { data } = await http.post('/api/v1/tasks', payload());
            router.push({ name: 'tasks.show', params: { id: data.data.id } });
        }
    } catch (e) {
